...
public class EventoMB implements Serializable {
	...
	public void init() {
		...
		if (this.eventos != null) {
			...
			for (Evento ev : this.eventos) {
				...
				DefaultScheduleEvent eventoDB = new DefaultScheduleEvent(ev.getDescricao(), ev.getInicio(),
						ev.getFim());
		...
		try {
			...
			if (getSelectedUsuarios().isEmpty() && getSelectedGrupos().isEmpty()) {
				...
				FacesMessage msg = new FacesMessage(FacesMessage.SEVERITY_ERROR,
						"Favor Adicionar 1 usuário ou grupo ao evento.", "");
				FacesContext.getCurrentInstance().addMessage(null, msg);
			...
			if (this.evento.getId() == null) {
				...
				DefaultScheduleEvent eventoDB = new DefaultScheduleEvent(this.evento.getDescricao(),
						this.evento.getInicio(), this.evento.getFim());
				...
				if (this.participantes != null) {
					...
					for (ParticipanteEvento par : this.participantes) {
						...
						if (par != null) {
							...
							sendEmailParticipantes(par.getEmail(), par.getNome(), this.evento.getDescricao(),
									this.evento.getInicio(), this.evento.getFim(), this.evento.getLocal());
	...
	public String cancelarEvent() {
		...
		// recuperar tambem os usuarios dos grupos

		if (getSelectedGrupos() != null && !getSelectedGrupos().isEmpty()) {

			for (Grupo gr : getSelectedGrupos()) {

				List<Usuario> usuarioGrupos = gr.getUsuarios();

				for (Usuario user : usuarioGrupos) {
	...
	public String updateEvent() {
		...
		// recuperar os usuarios dos grupos tambem
		if (getSelectedGrupos() != null && !getSelectedGrupos().isEmpty()) {
	...
	private void sendMessageFirebase(String token, String conteudo)
		...
		post.setHeader("Authorization",
				"key=AAAAg6OKGls:APA91bHCLYvN31Zk09s6FmLy5k6pFYGGj74Ah9JSSLlFAMVoxupEVBEe8MFMPAdfyuqw-TsSPdJ_fjmjUzuKFcNXTcDlDHnroM0kGQPt6RDjNpO2hA-rpOU7YTn44SOdMCp9l6fUErc0");
	...
	public void onEventSelect(SelectEvent selectEvent) {
		...
		// verifica na base de dados qual dos usuarios ja visualizou no app e confirmou
		// presença
